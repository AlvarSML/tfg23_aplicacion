...
class StatesService:
  ...
  @staticmethod
  def change_seg(db: Session, seg_model: int, current_user:models.User) -> schemas.StateInDBBase:
    """ Modifica el modelo de segementacion del último estado
    """
    last_state = crud.state.get_last(db)
    new_reg_model = crud.model.get_by_id(db, seg_model)

    if new_reg_model:
      last_state.seg_model = new_reg_model
      new_state = crud.state.create_with_owner(db=db, obj_in=last_state, owner_id=current_user.id)
      return new_state
    else:
      raise HTTPException(status_code=404, detail="Nuevo modelo no encontrado")